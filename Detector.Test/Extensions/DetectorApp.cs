...
namespace Detector.Test.Extensions;

internal class DetectorApp : WebApplicationFactory<Program>
{
    private readonly SqliteConnection _sqliteConnection = new("Filename=:memory:");

    public DetectorContext CreateDetectorContext()
    {
        var db = Services.GetRequiredService<IDbContextFactory<DetectorContext>>().CreateDbContext();
    ...
    public async Task CreateUserAsync(string username= "userid", string? password = null)
    {
        ...
        var newUser = new DetectorUser { UserName = username };
    ...
    protected override IHost CreateHost(IHostBuilder builder)
    {
        ...
        builder.ConfigureServices(services =>
        {
            ...
            services.AddDbContextFactory<DetectorContext>();

            // We need to replace the configuration for the DbContext to use a different configured database
            services.AddDbContextOptions<DetectorContext>(o => o.UseSqlite(_sqliteConnection));
